...
interface V2CareerValuesSectionProps {
  data?: {
    badge?: string;
    title?: string;
    subtitle?: string;
...
export const V2CareerValuesSection: React.FC<V2CareerValuesSectionProps> = ({ data, sectionId }) => {
  const badge = data?.badge ?? '4 PRINCIPLES';
  const title = data?.title ?? 'Our Core Values';
  const subtitle = data?.subtitle ?? "We don't just host data; we empower the people who build it.";
  const image = data?.image ?? 'https://c.animaapp.com/NzJCLB5d/img/sole.png';

  const { items: values, getItemProps, SortableWrapper } = useArrayItems<ValueItem>(
    'values',
    data?.values ?? []
  );
  ...
  return (
    <section
      ...
      aria-labelledby="v2-career-values-heading"
    >
      <div className="w-full max-w-[1920px] mx-auto flex flex-col lg:flex-row items-start lg:items-center gap-10 lg:gap-[120px] py-10 md:py-16 lg:py-[120px] px-4 sm:px-6 md:px-12 lg:px-16 xl:px-24 2xl:px-[120px]">
        ...
        <figure className="relative w-full lg:w-[640px] aspect-square rounded-[25px] overflow-hidden shrink-0">
          ...
          <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
            <img
              ...
              alt=""
              src="https://c.animaapp.com/NzJCLB5d/img/main.png"
            />
          </div>
        </figure>
...
export default V2CareerValuesSection;
