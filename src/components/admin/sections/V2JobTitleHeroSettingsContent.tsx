...
import type { V2JobTitleHeroSectionData } from '@/types/newSectionTypes';
import type { BaseSectionData } from '@/types/baseSectionTypes';

interface Props {
  data: V2JobTitleHeroSectionData & BaseSectionData;
  onChange: (data: V2JobTitleHeroSectionData & BaseSectionData) => void;
}

export function V2JobTitleHeroSettingsContent({ data, onChange }: Props) {
  ...
  const updateDetail = (index: number, field: string, value: string) => {
    ...
    updateField('jobDetails', details);
  };
      ...
      <div className="pt-4 border-t space-y-3">
        ...
        {(data.jobDetails || []).map((detail, i) => (
          <div key={i} className="space-y-1.5 border-b pb-3">
            ...
            <DebouncedInput value={detail.icon || ''} onChange={(v) => updateDetail(i, 'icon', v)} className="h-8 text-xs" debounceMs={300} />
          </div>
        ))}
      </div>
    </div>
...
export default V2JobTitleHeroSettingsContent;
